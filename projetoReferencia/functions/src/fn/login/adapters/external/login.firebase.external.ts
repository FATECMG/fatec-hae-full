...
@injectable()
export class LoginFirebaseExternal implements IExternal<ILogin, Auth> {
  async call(login: ILogin): Promise<Auth> {
    const auth = firebase.auth()
    auth.setPersistence(firebase.auth.Auth.Persistence.NONE)
    let user
    try {
      user = await auth.signInWithEmailAndPassword(login.email, login.password)
    } catch (err) {
