...
describe('DeactivateCourseMongoRepository', () => {
  let mongoServer: MongoMemoryServer
  let systemUnderTest: DeactivateCourseMongoRepository
  let testCourse: Course
  let createTestCourse: () => Promise<string>

  beforeAll(async () => {
    systemUnderTest = new DeactivateCourseMongoRepository()
  ...
  it('should return true if CourseModel.findOneAndUpdate() returns a deactivated course', async () => {
    const id = await createTestCourse()
