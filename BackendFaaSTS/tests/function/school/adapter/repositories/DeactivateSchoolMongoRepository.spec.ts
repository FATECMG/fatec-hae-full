...
describe.only('DeactivateSchoolMongoRepository', () => {
  let mongoServer: MongoMemoryServer
  let systemUnderTest: DeactivateSchoolMongoRepository
  let testSchool: School
  let createTestSchool: () => Promise<string>

  beforeAll(async () => {
    systemUnderTest = new DeactivateSchoolMongoRepository()
  ...
  it('should calls SchoolModel.findOneAndUpdate() with correct values', async () => {
    ...
    expect(findOneAndUpdateSpy).toHaveBeenCalledWith({ id }, { active: false }, { new: true })
  })

  it('should return true if SchoolModel.findOneAndUpdate() returns a deactivated school', async () => {
    const id = await createTestSchool()
